...
export function buildSendImageTool(opts: {
  ...
  return defineTool({
    ...
    description: [
      ...
      '图片来源：① 本地已存在的图片文件，或截图 / 下载等工具返回的图片路径——直接传其绝对路径，或相对你当前命令工作目录（与 bash 等命令工具所在目录一致，可用 pwd 查看）的相对路径；',
      '② 若工具只返回 base64 数据，先用 mcpScript 等工具写入文件（建议保存到你命令工作目录下）再传路径，勿把 base64 或 data URL 当作 target；③ http(s) 图片 URL。',
      `单张图片大小不能超过 ${maxMb}MB，大图先压缩再发送。`,
